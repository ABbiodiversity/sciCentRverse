...
- reorder:
  ...
  logical. If TRUE, the chosen response column is moved to the front of
  the new data frame. Defaults to TRUE.

## Value

A data frame that includes only the chosen response column (plus all
columns that were not in `response_vars`).
